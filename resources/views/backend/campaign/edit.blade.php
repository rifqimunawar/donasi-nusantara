...
@section('content')
    ...
    <div>
        <div class="overflow-x-auto">
          ...
          <form method="post" action="{{ route('campaign.update', ['id' => $campaign->id]) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-5">
                ...
                <select name="category_id" required class="rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full">
                    <option disabled>== Pilih Kategori ==</option>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ $campaign->category_id == $category->id ? 'selected' : '' }}>
